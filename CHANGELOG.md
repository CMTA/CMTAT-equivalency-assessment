...
- Document Version section: distinction between the **template version** (this document, as published by CMTA) and the **assessment version** (the filled document, set by its author)
  - Table with the two values: the template version, pre-filled and not to be modified, and the assessment version, set by the author. Every assessment MUST fill it, including one written as a separate report, which reproduces the table with it
