...
- `CMTAT_SUGGESTION_EDITORIAL.md`: companion suggestion document on the editorial side of the framework specifications — giving the document its own version number and making the functionality numbers stable across editions, then the typographical defects with the page, the current text and a suggested correction for each, plus two consistency points on module naming and on the role symbols
