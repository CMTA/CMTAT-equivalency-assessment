...
- `CMTAT_SUGGESTION.md`: suggested improvements to the CMTAT Framework functional specifications (June 2026 edition), written from the mapping work behind the criteria
  - Twelve groups of suggestions, each giving the current wording with its section and page, the gap, and a proposed change: versioning, cross-chain transferability, transfer restrictions, enforcement and cancellation, pause and deactivation, roles, attributes and documents, batch operations, auditability, reference implementations, divergences with these criteria, and editorial corrections
