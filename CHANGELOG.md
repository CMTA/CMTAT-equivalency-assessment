...
- `CMTAT_SUGGESTION.md`: suggested improvements to the CMTAT Framework functional specifications (June 2026 edition), written from the mapping work behind the criteria
  ...
  - § 9 suggests splitting the framework's Base module, whose eleven mandatory functionalities cover four unrelated concerns, into a Base module (the attributes), a Token module (supply, balance, transfer, decimals), a Supply module (create and cancel) and a Pause module (pause, unpause, deactivate and the two status reads) — a grouping that matches neither § 4.1 of the framework itself nor the Solidity implementation, and that obscures where a role or a restriction attaches. It carries the point that the module names in the chain-agnostic sections are functional names and not Solidity contract names, which moved here from the editorial companion because splitting the module makes the two lists correspond more closely
