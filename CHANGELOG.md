...
- Document Version section: distinction between the **template version** (this document, as published by CMTA) and the **assessment version** (the filled document, set by its author)
  - Table with the two values: the template version, pre-filled and not to be modified, and the assessment version, set by the author. An assessment written as a separate report, rather than by filling this file, quotes both on a single line: `v0.2.0 (this assessment), filled from CMTA assessment template v0.3.0`
