...
- That the cancellation on the source ledger MAY additionally be made to depend on an **authorisation given by the holder** for its own tokens. CMTAT Solidity offers both bounds in the same module: `crosschainBurn` requires no such authorisation and can cancel from any address, while `burnFrom` spends an allowance the holder has granted. The two are complementary — a period limit caps the aggregate a compromised bridge can cancel, a holder authorisation caps what it can take from each holder — and the framework SHOULD present them as such rather than as alternatives.
...
- The cancellation on the source ledger MAY be made to depend on an authorisation given by the holder for its own tokens, in addition to the authorisation of the bridge. Where it is, a bridge cannot cancel more than each holder has authorised, and cannot cancel at all from a holder who has authorised nothing. This bound and the limit under functionality 56 apply to different quantities — one to what a bridge may take from each holder, the other to the aggregate over a period — and an implementation MAY apply both. The cost is that the holder gives that authorisation before the transfer, which adds an operation to the cross-chain path and does not suit an arrangement in which the bridge acts without the holder.
