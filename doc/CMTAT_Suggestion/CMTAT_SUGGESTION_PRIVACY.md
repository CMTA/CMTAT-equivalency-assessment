...
- **The counterparty learns it.** A holder who sends tokens to a frozen address, or a frozen holder who sends tokens, has the operation rejected by the ledger. Where the sender knows that the other conditions are met — the token is not paused, both addresses are on the whitelist, the balance is sufficient — the freeze is the only remaining explanation, and the probe costs no more than one rejected transaction.
- **Third parties may learn it as well.** Where the ledger publishes the fact that a transaction was rejected even without publishing its contents, the counterparty is not the only person who learns it.
- **What restricting the reading still buys.** It prevents the status from being enumerated over addresses at large, and prevents anyone who attempts no transfer from obtaining it. It does not conceal the status from a counterparty who tries, and the same probe identifies an address missing from a whitelist.
...
#### § 2.8.1 Items of data requiring particular care

**The number of tokens in circulation.** It is an aggregate of every balance, and on a confidential ledger it opens two inference channels:

- **The difference between two readings.** An observer who obtains the figure at two points in time can determine the net number of tokens created or cancelled between them, and where a single such operation occurred in that interval, its amount is revealed in full.
- **The complement within a small body of holders.** A holder who knows the figure and its own balance also knows the aggregate held by all the other holders, which, where the holders are few, narrows their individual balances.

An implementation MAY therefore restrict the reading of that figure, including as against holders, where the confidentiality of individual operations is to be preserved. It MUST then state how a holder determines the extent of a right expressed as a proportion of the tokens in issue. Where the figure is disclosed periodically, the disclosures SHOULD cover several operations rather than each one.

**The frozen status of an address.** It is subject to a comparable limit, which arises from the way a freeze takes effect rather than from the way the status is stored:

- **The counterparty learns it.** A freeze takes effect by causing the transfer to be rejected, so any person who attempts a transfer to or from a frozen address learns that the transfer is not permitted. Where that person knows that the other conditions of the transfer are satisfied, the freeze is disclosed to them by the rejection alone.
...
- Its reading SHOULD be open to any person having access to the ledger, and an implementation on a confidential ledger has no reason to protect it.
- That open reading is a recommendation and not a requirement, since a ledger may restrict access at its own level, and an implementation is not to be treated as non-compliant because the ledger it uses is permissioned.
- What is required is that the issuer and every token holder be able to read the value, without which a holder cannot interpret its own balance.

#### § 2.8.2 Obligations of the implementation
